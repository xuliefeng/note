public class ConcurrentHashMap<K, V> extends AbstractMap<K, V>
    ...
    final V replaceNode(Object key, V value, Object cv) {
        ...
        for (Node<K, V>[] tab = table; ; ) {
            ...
            if (tab == null
                    || (n = tab.length) == 0
                    || (f = tabAt(tab, i = (n - 1) & hash)) == null) break;
            else if ((fh = f.hash) == MOVED) tab = helpTransfer(tab, f);
            else {
                V oldVal = null;
                    ...
                    if (first.hash < 0) {
                        ...
                        if (insertAtFront && binCount >= TREEIFY_THRESHOLD) {
                            ...
                            for (q = p; q != null; q = q.next) {
                                TreeNode<K, V> t =
                                        new TreeNode<K, V>(q.hash, q.key, q.val, null, null);
        ...
        Node<K, V> find(int h, Object k) {
            ...
            for (Node<K, V>[] tab = nextTable; ; ) {
                ...
                if (k == null
                        || tab == null
                        || (n = tab.length) == 0
                        || (e = tabAt(tab, (n - 1) & h)) == null) return null;
                for (; ; ) {
                    int eh;
                    K ek;
    ...
    private final void addCount(long x, int check) {
        CounterCell[] as;
        long b, s;
        // counterCells不为空（有并发）或计数数量CAS更新失败
    ...
    private final void fullAddCount(long x, boolean wasUncontended) {
        int h; // 线程指针探针
        // 获取线程探针，如果等于0则初始化获取。并且将wasUncontended设置为true标明当前无竞争
                    ...
                    if (((s = lockState) & (WAITER | WRITER)) != 0) {
                        ...
                        try {
                            ...
                            if (U.getAndAddInt(this, LOCKSTATE, -READER) == (READER | WAITER)
                                    && (w = waiter) != null) LockSupport.unpark(w);
                        }
                        return p;
                    }
                }
                ...
                if (p == null) {
                    ...
                    if (!searched) {
                        ...
                        if (((ch = p.left) != null && (q = ch.findTreeNode(h, k, kc)) != null)
                                || ((ch = p.right) != null
                                        && (q = ch.findTreeNode(h, k, kc)) != null)) return q;
                    }
                    dir = tieBreakOrder(k, pk);
                }
    ...
    static class BaseIterator<K, V> extends Traverser<K, V> {
        ...
        BaseIterator(
                Node<K, V>[] tab, int size, int index, int limit, ConcurrentHashMap<K, V> map) {
    ...
    static final class ValueIterator<K, V> extends BaseIterator<K, V>
        ...
        ValueIterator(
                Node<K, V>[] tab, int index, int size, int limit, ConcurrentHashMap<K, V> map) {
            super(tab, index, size, limit, map);
        }
    ...
    public <U> void forEach(
        ...
        new ForEachTransformedMappingTask<K, V, U>(
                        null, batchFor(parallelismThreshold), 0, 0, table, transformer, action)
                .invoke();
    }
    ...
    public K reduceKeys(
        ...
        return new ReduceKeysTask<K, V>(
                        null, batchFor(parallelismThreshold), 0, 0, table, null, reducer)
                .invoke();
    }
    ...
    public <U> U searchEntries(
            long parallelismThreshold, Function<Map.Entry<K, V>, ? extends U> searchFunction) {
        if (searchFunction == null) throw new NullPointerException();
